...
def open_accordant_config():
    device_type = os.popen('adb shell getprop ro.product.model').read().strip()
    screen_size = _get_screen_size()
    config_file = "{path}/config/{screen_size}/{os}config.json".format(
        path=sys.path[0],
        screen_size=screen_size,
        os=device_type.replace(" ", "").lower() + '_' if True or device_type is "MI 5" else ""
    )
    print "open_accordant_config method-->config_file is %s" % (config_file)
...
def pull_screenshot():
    ...
    screenshot = process.stderr.read()
...
def pull_screenshot2():
    screencap = "adb shell screencap -p /sdcard/autojump.png"
    pull = r"adb pull /sdcard/autojump.png D:\software\preinstall\DevelopTool\python\demo\jump\autojump.png"
    process = subprocess.Popen(screencap, shell=True)
    time.sleep(2)
    close_process(process, 1)
    process2 = subprocess.Popen(pull, shell=True)
    time.sleep(1)
    close_process(process2, 2)
...
def jump(distance):
    ...
    if distance < 330 and distance > 190:
        press_time = distance * 1.93
        print "distance is %s, press_base is 1.93" % (distance)
    else:
        press_time = distance * press_coefficient
        print "distance is %s, press_base is %s" % (distance, press_coefficient)
    press_time = max(press_time, 200)   # 设置 200 ms 是最小的按压时间
    press_time = int(math.ceil(press_time)+1)
